...
function getAlertHistories() {
    $(".widget-settings-initializing").html("Initializing Alert Histories");
    $.ajax({
        type: "GET",
        url: "/api/alert-histories/",
        data: {},
        dataType: "json",
       	success: function(data) {
       	    var json_data = eval(data);
            appVars.alert_histories = json_data;
            getProcessData();
       	},
       	error: function(jqXHR, textStatus, errorThrown) {
            getProcessData();
...
function getProcessData() {
    $(".widget-settings-initializing").html("Initializing Process Data");
    $.ajax({
        type: "GET",
...
function initializeWidgets() {
    ...
    $(".div-container .div-container").click(function() {
        // Get widget data...
        ...
        // Set new data...
        widgetData.config.nickname = nickname;
    ...
    $("div.widget").each(function(index) {
        ...
        switch (widgetData.type) {
            case "cpu":
                displayCPUAlerts(widgetData, true);
                break;
            
            case "memory":
                displayMemoryAlerts(widgetData, true);
                break;
...
function displayWidgetInterval() {
    $("div.widget").each(function(index) {
        ...
        if (widgetData.timerIntervalCounter >= widgetData.timerInterval) {
            ...
            switch (widgetData.type) {
                case "cpu":
                    displayCPUAlerts(widgetData, false);
                    break;
            
                case "memory":
                    displayMemoryAlerts(widgetData, false);
                    break;
